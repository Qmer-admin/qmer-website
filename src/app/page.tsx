...
export default function Home() {
  // --- MANTIK KATMANI ---
  // Tüm ürünleri değil, sadece "isNew" olanları veya ilk 3'ü seçiyoruz.
  // Bu sayede data.ts'de bir ürüne "isNew: true" dediğin an burada çıkar.
  const newArrivals = products.filter((p) => p.isNew).slice(0, 3);

  // Eğer hiç yeni ürün yoksa, boş kalmasın diye ilk 3 ürünü alalım (Fallback)
  const displayProducts = newArrivals.length > 0 ? newArrivals : products.slice(0, 3);

  return (
    <div className="bg-[#FDFCF8]">
      
      {/* --- HERO ALANI (Senin Kodun) --- */}
      <section className="relative h-[95vh] flex items-center justify-center overflow-hidden">
         
         {/* 1. KATMAN: Arka Plan */}
         <div className="absolute inset-0 z-0">
            {/* Eğer bu resim yoksa gradient görünür, sorun olmaz */}
            <div className="absolute inset-0 bg-stone-900 z-0" /> 
            <Image 
                src="/mediterranean-bg.jpg" 
                alt="Mediterranean Coast" 
                fill
                className="object-cover opacity-80" // Hafif kıstım yazılar patlasın diye
                priority
            />
         </div>

         {/* 2. KATMAN: Gradient Overlay (Senin Sihirli Dokunuşun) */}
         ...
         <div className="container mx-auto px-4 text-center max-w-4xl relative z-20 pt-32 animate-fadeIn">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-5">
                <Link href="/orweym" className="bg-emerald-900 text-white px-10 py-4 rounded-sm font-semibold text-xs uppercase tracking-widest hover:bg-emerald-800 transition-all duration-300 shadow-xl shadow-emerald-900/20 transform hover:-translate-y-1">
                    Discover Collection
                </Link>
      ...
      {/* --- NEW ARRIVALS (Revize Edildi) --- */}
      <section className="container mx-auto px-6 pb-32 pt-20">
        {/* Başlık ve "View All" Butonu Yan Yana */}
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6 border-b border-stone-200 pb-6">
            ...
            <Link href="/orweym" className="group flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-stone-500 hover:text-emerald-800 transition-colors">
              View Full Collection
              <span className="block w-6 h-px bg-stone-300 group-hover:bg-emerald-800 transition-colors"></span>
            </Link>
        </div>
        
        {/* Sadece Yeni Ürünler Grid */}
      ...
      {/* --- GÜVEN BANDI (Senin Kodun - Dokunulmadı) --- */}
      <section className="bg-[#1C1C1C] text-white py-24 relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/50"></div> {/* Görsel varsa üstünü karartmak için */}
